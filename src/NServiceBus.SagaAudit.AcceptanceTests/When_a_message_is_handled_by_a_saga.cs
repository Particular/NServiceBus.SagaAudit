...
public class When_a_message_is_handled_by_a_saga : NServiceBusAcceptanceTest
{
    ...
    public async Task Should_populate_InvokedSagas_header_for_single_saga_audits()
    {
        var context = await Scenario.Define<Context>()
            ...
            .WithEndpoint<EndpointWithASaga>(b => b.When((messageSession, ctx) =>
                messageSession.SendLocal(new MessageToBeAudited { Id = ctx.TestRunId })
            ))
    ...
    public async Task Should_populate_InvokedSagas_header_for_multiple_saga_audits()
    {
        var context = await Scenario.Define<Context>()
            .WithEndpoint<FakeServiceControl>()
            .WithEndpoint<EndpointWithASaga>(b => b.When((messageSession, ctx) =>
                messageSession.SendLocal(new MessageToBeAuditedByMultiple { Id = ctx.TestRunId })
            ))
            .Done(c => c.MessageAudited)
            .Run();
    ...
    class EndpointWithASaga : EndpointConfigurationBuilder
    {
        ...
        public class TheEndpointsSaga(Context testContext) : Saga<TheEndpointsSagaData>, IAmStartedByMessages<MessageToBeAudited>, IAmStartedByMessages<MessageToBeAuditedByMultiple>
        {
        ...
        public class TheEndpointsSagaAlternative(Context testContext) : Saga<TheEndpointsSagaAlternativeData>, IAmStartedByMessages<MessageToBeAuditedByMultiple>
        {
            protected override void ConfigureHowToFindSaga(SagaPropertyMapper<TheEndpointsSagaAlternativeData> mapper) => mapper.MapSaga(s => s.TestRunId).ToMessage<MessageToBeAuditedByMultiple>(msg => msg.Id);
    ...
    class FakeServiceControl : EndpointConfigurationBuilder
    {
        ...
        {
            public Task Handle(SagaUpdatedMessage message, IMessageHandlerContext context) => Task.CompletedTask;
        }

        public class MessageToBeAuditedHandler(Context testContext) : IHandleMessages<MessageToBeAudited>, IHandleMessages<MessageToBeAuditedByMultiple>
        {
            public Task Handle(MessageToBeAudited message, IMessageHandlerContext context)
            {
